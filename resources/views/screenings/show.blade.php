            ...
            @foreach ($seats->groupBy('row') as $row => $seatsByRow)
                <div class="flex justify-center gap-2">
                    @foreach ($seatsByRow as $seat)
                        <div class="text-center seat" data-seat-id="{{ $seat->id }}"
                            data-available="{{ $seat->isAvailable($screening->id) ? '1' : '0' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                width="60px" height="60px" viewBox="0 0 600 600" version="1.1">
                                <g id="surface1">
                                    <path
                                        style="stroke:black;fill:{{ !$seat->isAvailable($screening->id) ? 'rgb(231, 77, 77)' : 'black' }};
                                    fill-opacity:1;"
                            ...
                            <h1 class="text-center"
                                style="color: {{ !$seat->isAvailable($screening->id) ? 'red' : 'black' }}">
                                {{ $seat->seatName }}
                            </h1>
                        </div>
                    @endforeach
